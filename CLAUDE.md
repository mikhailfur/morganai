...
4. TODO.md — единственный источник задач и приоритетов:
   - В начале каждой сессии читать `TODO.md` и ориентироваться на раздел «В работе», затем «Очередь».
   - По мере выполнения задачи — переносить её в раздел «Готово» с пометкой `[x]`.
   - Если пользователь добавляет задачу в чат, предложить внести её в `TODO.md` в нужный раздел.
   - Не начинать задачи из «Идей» без явного подтверждения пользователя.
...
- Гадать по поводу API/форматов внешних сервисов — если что-то не работает или поведение неожиданное, **сначала читать документацию** (ссылки в разделе «Документация» ниже) через WebFetch, а не перебирать варианты вслепую. Пример: неверный формат запроса к OpenRouter STT был обнаружен именно через доку, а не методом проб.
