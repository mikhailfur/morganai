...
async function initGoogleButton() {
    ...
    try {
        ...
        /* GSI load error, button stays as-is */
    }
}

onMounted(() => initGoogleButton());
watch(
    () => auth.appConfig.googleClientId,
    () => initGoogleButton(),
);
...
async function handleTelegramOAuth() {
    ...
    if (!telegramBotId) {
        error.value =
            "Telegram OAuth не настроен — укажи TELEGRAM_BOT_TOKEN в env";
        return;
    }
    try {
        ...
        (window as any).Telegram.Login.auth(
            ...
            async (user: Record<string, any> | null) => {
                if (!user) {
                    error.value = "Авторизация Telegram отменена";
                    return;
                }
                try {
...
<template>
    ...
    >
        ...
        >
            ...
            >
                ...
                >
                    <img
                        :src="'/logo.png'"
                        alt="Morgan"
                        style="height: 36px; border-radius: 6px; display: block"
                    />
                    <span
                        style="
                            font-family: var(--font-display);
            ...
            <div
                class="editorial-label animate-fade-in"
            ...
            <div
                class="display-heading animate-fade-in-1"
                style="font-size: clamp(40px, 5vw, 58px); margin-top: 14px"
            >
                С
                <span style="font-style: italic; color: var(--accent3)"
                    >возвращением.</span
                >
            </div>
            <p
                class="animate-fade-in-1"
                style="
                    font-family: var(--font-display);
                    font-size: 16px;
                    color: var(--fg);
            ...
            >
                ...
                <button
                    type="submit"
                    class="btn-primary"
                    style="width: 100%; padding: 16px; margin-top: 8px"
            ...
            <div
                style="
                    ...
                    gap: 10px;
                "
                class="animate-fade-in-3"
            >
                <!-- Google button: our styled visual + invisible Google iframe overlay -->
                <div
                    style="
                        position: relative;
                        overflow: hidden;
                ...
                >
                    ...
                    >
                        <path
                            d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.894 8.221l-1.97 9.28c-.145.658-.537.818-1.084.508l-3-2.21-1.447 1.394c-.16.16-.295.295-.605.295l.213-3.053 5.56-5.023c.242-.213-.054-.333-.373-.12L7.17 13.7l-2.94-.916c-.64-.203-.654-.64.135-.949l11.566-4.461c.537-.194 1.006.131.963.847z"
                        />
                    </svg>
                    Telegram
                </button>
